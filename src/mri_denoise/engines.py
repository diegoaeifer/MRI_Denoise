...
from monai.handlers import (
    ...
    ValidationHandler, CheckpointSaver, LrScheduleHandler,
    MeanSquaredError, PeakSignalToNoiseRatio, SSIMHandler,
    from_engine
)
...
def build_evaluator(network, val_loader, device, cfg):
    val_handlers = [
        ...
        TensorBoardStatsHandler(log_dir=cfg.get("log_dir", "logs"), output_transform=lambda x: None),
        CheckpointSaver(
    ...
    return SupervisedEvaluator(
        ...
        additional_metrics={
            "val_mse": MeanSquaredError(output_transform=from_engine(["pred", "label"])),
...
def build_trainer(network, loss_fn, optimizer, scheduler, train_loader, evaluator, device, cfg):
    train_handlers = [
        LrScheduleHandler(scheduler, print_lr=True),
        ValidationHandler(validator=evaluator, interval=cfg.get("val_interval", 1), epoch_level=True),
