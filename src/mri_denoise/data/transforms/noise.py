...
class SpatiallyVaryingNoised(MapTransform):
    ...
    """
    def __init__(self, keys, sigma_range=(0.01, 0.1), grid_size=4,
                 multiplier_range=(0.5, 3.0), noise_type='gaussian',
                 spatial_dims=2, allow_missing_keys=False):
    ...
    def __call__(self, data):
        ...
        for key in self.key_iterator(d):
            ...
            if self.spatial_dims == 3:
                mod_grid = torch.empty(1, 1, self.grid_size, self.grid_size, self.grid_size, dtype=img.dtype, device=img.device).uniform_(*self.multiplier_range)
                size = img.shape[1:]  # H, W, D
                mod_map = F.interpolate(mod_grid, size=size, mode='trilinear', align_corners=False)
                mod_map = mod_map.squeeze(0)  # (1, H, W, D)
            else:
                # 2D or 2.5D (slice stacking)
                mod_grid = torch.empty(1, 1, self.grid_size, self.grid_size, dtype=img.dtype, device=img.device).uniform_(*self.multiplier_range)
                size = img.shape[-2:]  # H, W
                mod_map = F.interpolate(mod_grid, size=size, mode='bilinear', align_corners=False)
