...
class UniRepLKNetBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.with_cp and inputs.requires_grad:
            out = torch.utils.checkpoint.checkpoint(_f, inputs)
        else:
            out = _f(inputs)
        return out
...
class DB_Conv(nn.Module):
    def __init__(self, inChannels, growRate, kernel_size=3):
        ...
        self.conv = nn.Sequential(
            *[
                nn.Conv2d(
                    n_feats, rate, kernel_size, padding=(kernel_size - 1) // 2, stride=1
                ),
                nn.ReLU(),
            ]
        )
...
class BioInspiredLittleAddInhibitionBlock(nn.Module):
    def __init__(self, in_channel, inner_channel, out_channel, strides=1):
        ...
        self.ParasolCells = nn.Sequential(
            nn.Conv2d(
                in_channel, inner_channel, kernel_size=1, stride=strides, padding=0
            ),
            nn.ReLU(inplace=True),
            nn.Conv2d(inner_channel, out_channel, kernel_size=1, stride=strides),
            nn.ReLU(inplace=True),
...
class DPN(nn.Module):
    def __init__(self, in_channels=2, out_channels=1, spatial_dims=2):
        ...
        self.pdense2 = Pdense_conv(32, in_channels)
        ...
        self.dorsal_conv1 = Dorsal_conv(in_channels, 16)
