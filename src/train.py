...
def main(args):
    ...
    if args.train_data_dir and args.val_data_dir:
        ...
        train_loader_obj = NiftiLoader(
            data_path=args.train_data_dir, seed=config["data"]["seed"], limit=args.limit
        )
        val_loader_obj = NiftiLoader(
            data_path=args.val_data_dir, seed=config["data"]["seed"], limit=args.limit
        )
    ...
    logger.info(
        f"Augmentation Configs: {config.get('data', {}).get('augmentation', {})}"
    )
    ...
    for epoch in range(config["training"]["epochs"]):
        ...
        metrics_str = ", ".join(
            [f"{k.upper()}: {v:.4f}" for k, v in val_metrics.items() if v != 0.0]
        )
        logger.info(
            f"Epoch {epoch+1}: Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, {metrics_str}"
        )
        ...
        # Visuals
        if (epoch + 1) % config["training"].get(
            "log_visuals_freq", 5
        ) == 0 or args.test:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="drunet",
        help="Model architecture to train (drunet, nafnet, scunet, unet, drunet_pretrained, etc.)",
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of images for debugging"
    )
    parser.add_argument(
        "--test",
        action="store_true",
        help="Run in test mode with specific data and overrides",
    )
    parser.add_argument(
        "--data_dir", type=str, default=None, help="Override base data directory"
    )
    parser.add_argument(
        "--train_data_dir",
        type=str,
        default=None,
        help="Specific training data directory for NIFTI workflow",
    )
    parser.add_argument(
        "--val_data_dir",
        type=str,
        default=None,
        help="Specific validation data directory for NIFTI workflow",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Override base output directory for logs/checkpoints",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
