...
def process_batch(file_paths, threshold_range=1000):
    ...
    for fp in file_paths:
        try:
            ...
            # and use in-place clipping for consistency with the training pipeline
            ...
            # Use in-place clipping to handle outliers before calculating dynamic range
            np.clip(image, p1, p99, out=image)
...
def check_background_batched(
    data_path, threshold_range=1000, batch_size=500, auto_delete=False, workers=None
):
    logger.info(
        f"Scanning {data_path} (Batch Size: {batch_size}, Auto-Delete: {auto_delete}, Workers: {workers})"
    )
    ...
    batches = [
        all_dcm_files[i : i + batch_size] for i in range(0, total_files, batch_size)
    ]

    # Process in batches in parallel
    with ProcessPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(process_batch, batch, threshold_range): batch
    ...
    if problematic_all:
        ...
        with open("background_report.txt", "w") as f:
            f.write(
                f"Background Report (Range Threshold: {threshold_range}, Auto-Deleted: {auto_delete})\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--threshold", type=float, default=1000, help="Dynamic range threshold"
    )
    parser.add_argument("--batch_size", type=int, default=1000)
    parser.add_argument(
        "--delete", action="store_true", help="Delete without confirmation"
    )
    parser.add_argument(
    ...
    check_background_batched(
        args.data_path, args.threshold, args.batch_size, args.delete, args.workers
    )
