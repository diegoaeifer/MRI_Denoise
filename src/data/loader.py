...
class DICOMLoader:
    ...
    def scan_directory(self):
        ...
        if self.cache and cache_file.exists():
            ...
            try:
                ...
                patient_registry = {
                    pid: set(sids)
                    for pid, sids in cached_data["patient_registry"].items()
                }
                series_registry = cached_data["series_registry"]

                logger.info(
                    f"Loaded {len(patient_registry)} unique patients from cache."
                )
                ...
                if file.lower().endswith(".dcm") or "." not in file:
                    ...
                    try:
                        ...
                        ds = pydicom.dcmread(
                            ...
                            specific_tags=["PatientID", "SeriesInstanceUID"],
                        )
        ...
        logger.info(
            f"Found {len(patient_registry)} unique patients and {len(series_registry)} unique series."
        )
        ...
        if self.cache:
            try:
                ...
                serializable_registry = {
                    pid: list(sids) for pid, sids in patient_registry.items()
                }
                cache_data = {
    ...
    def create_splits(self, output_dir=None):
        ...
        if self.limit:
            ...
            logger.info(
                f"Applied limit {self.limit} -> Train: {len(splits['train'])}, Test: {len(splits['test'])}, Val: {len(splits['val'])}"
            )
        ...
        if output_dir:
            ...
            for split_name, files in splits.items():
                ...
                logger.info(
                    f"Saved {split_name} split with {len(files)} files to {out_path}"
                )
