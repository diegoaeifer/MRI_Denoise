...
class MRI_DICOM_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            is_nifti = str(file_path).lower().endswith(".nii") or str(
                file_path
            ...
            p_min, p_max = np.quantile(image[::stride, ::stride], [q_min, q_max])
            ...
            if noisy_image.ndim not in [3, 4]:
                raise ValueError(
                    f"Expected 3D (1, H, W) or 4D (1, H, W, D) noisy_image, got {noisy_image.shape}"
                )
